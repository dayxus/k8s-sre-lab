...
from invariants import INVARIANT_DESCRIPTIONS, INVARIANTS, check_all, count_by_invariant
...
def docs_for(shape: str, request) -> list:
    return request.getfixturevalue(
        {"base": "base_docs", "dev": "dev_docs", "prod": "prod_docs"}[shape]
    )
...
def test_deployment_probes_have_distinct_jobs(base_docs):
    container = find(base_docs, "Deployment", "demo-api")["spec"]["template"]["spec"]["containers"][
        0
    ]
...
def test_spread_and_anti_affinity_are_soft_on_purpose(base_docs):
    ...
    anti = pod_spec["affinity"]["podAntiAffinity"][
    ...
    assert (
        ...
        not in pod_spec["affinity"]["podAntiAffinity"]
    )
...
def test_alert_expressions_reference_metrics_the_app_exposes(base_docs):
    ...
    app_source = (
        __import__("pathlib").Path(__file__).resolve().parents[1] / "app" / "server.py"
    ).read_text()
    rule = find(base_docs, "PrometheusRule", "demo-api")
    for group in rule["spec"]["groups"]:
        for rule_entry in group["rules"]:
            for metric in (
                "demo_api_ready_failures_total",
...
def test_prod_requests_fit_inside_limits_and_grow_with_the_environment(dev_docs, prod_docs):
    dev_requests = find(dev_docs, "Deployment", "demo-api")["spec"]["template"]["spec"][
        "containers"
    ][0]["resources"]["requests"]
    prod_container = find(prod_docs, "Deployment", "demo-api")["spec"]["template"]["spec"][
        "containers"
    ][0]
...
def test_overlays_do_not_restate_the_contract(dev_docs, prod_docs, base_docs):
    ...
    base_container = find(base_docs, "Deployment", "demo-api")["spec"]["template"]["spec"][
        "containers"
    ][0]
    for docs in (dev_docs, prod_docs):
        container = find(docs, "Deployment", "demo-api")["spec"]["template"]["spec"]["containers"][
            0
        ]
        for field in (
            "readinessProbe",
